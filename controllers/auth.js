...
exports.login = asyncHandler(async (req, res, next) => {
    ...
    //validate email & pw
    if (!email || !password) {
        return next(new ErrorResponse('Please provice an email and password', 400));
    }
...
exports.forgotPassword = asyncHandler(async (req, res, next) => {
    ...
    const message = `make a put request to ${resetUrl} in order to reset password`;
    try {
        await mailSender({
            email: user.email,
            subject: 'forgot pw email',
            message
        });
